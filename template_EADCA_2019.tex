...
\selectlanguage{english}
   ...
   Neste trabalho descrevemos uma abordagem que busca extrair da fala humana determinados atributos acústicos que, ricos em informação sobre a expressão afetiva na voz, são passíveis de codificação visual nas formas tipográficas de uma transcrição dessa mesma fala. Foi feito um experimento que mediu as preferências dos participantes por diferentes modulações tipográficas quando estas são relacionadas a diferentes qualidades expressivas da voz (raiva, alegria, neutralidade, tristeza e surpresa). Encontramos indícios de que {\itshape peso} é uma modulação tipográfica adequada para representar falas entoadas com alta intensidade. Inversamente, {\itshape baseline shift} é apropriada para representar a melodia na voz quando esta for entoada com pouca intensidade.
   
   \end{abstract}

  \keywords{prosódia visual, tipografia, computação afetiva}
